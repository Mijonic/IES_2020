...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        public ResourceDescription GetValuesForTapChangerControlWithMaxGid()
        {
            long globalId = GetFirstElement(ModelCode.TAPCHANGERCONTROL, true);

            string message = "Getting values for TapChangerContro method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                Console.WriteLine($"Getting values for all TapChangerControlProperties. ( number of properties = {properties.Count} )");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all TapChangerControlProperties. (number of properties = {properties.Count}");


                rd = GdaQueryProxy.GetValues(globalId, properties);

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetValuesForTapChangerControl1_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                rd.ExportToXml(xmlWriter);
                xmlWriter.Flush();

                message = "Getting values for TapChangerControl method successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting values for TapChangerControl method for entered id = {0} failed.\n\t{1}", globalId, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public ResourceDescription GetValuesForTerminalWithMinGid()
        {
            long globalId = GetFirstElement(ModelCode.TERMINAL, false);

            string message = "Getting values for Terminal method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                Console.WriteLine($"Getting values for all TerminalProperties. (number of properties = {properties.Count}");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all TerminalProperties. (number of properties = {properties.Count}");


                rd = GdaQueryProxy.GetValues(globalId, properties);

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetValuesForTerminalWithMinGid_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                rd.ExportToXml(xmlWriter);
                xmlWriter.Flush();

                message = "Getting values for Terminal method successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting values for Terminal method for entered id = {0} failed.\n\t{1}", globalId, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetExtentValuesForCurveData()
        {

            ModelCode modelCode = ModelCode.CURVEDATA;

            string message = "Getting extent values for CurveData method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                Console.WriteLine($"Getting values for all CurveDataProperties. ( number of properties = {properties.Count} )");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all CurveDataProperties. (number of properties = {properties.Count}");

                iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties);
                resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);


                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetExtentValuesCurveData_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting extent values for CurveData method successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            {
                message = string.Format("Getting extent values for CurveData method failed for {0}.\n\t{1}", modelCode, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetExtentValuesForSynchronousMachine()
        {

            ModelCode modelCode = ModelCode.SYNCHRONOUSMACHINE;

            string message = "Getting extent values for SynchronousMachine method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                Console.WriteLine($"Getting values for all SynchronousMachineProperties. ( number of properties = {properties.Count} )");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all SynchronousMachineProperties. (number of properties = {properties.Count}");

                iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties);
                resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);


                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetExtentValuesSynchronousMachine_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting extent values for SynchronousMachine method successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            {
                message = string.Format("Getting extent values for SynchronousMachine method failed for {0}.\n\t{1}", modelCode, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetReactiveCapabilityCurveForSyncMachine()
        {

            long sourceGlobalId = GetFirstElement(ModelCode.SYNCHRONOUSMACHINE, false);

            string message = "Getting ReactiveCapabilityCurve assigned to SynchronousMachine started.";
            ...
            association.PropertyId = ModelCode.SYNCHRONOUSMACHINE_REACTCAPABILCURV;
            association.Type = 0;
            ...
            try
            {
                ...
                List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds(ModelCode.REACTIVECAPABILITYCURVE);

                Console.WriteLine($"Getting values for all ReactiveCapabilityCurveProperties. ( number of properties = {properties.Count} )");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all ReactiveCapabilityCurveProperties. (number of properties = {properties.Count}");


                Console.WriteLine("\nSyncronousMachine -> 0..1 ReactiveCapabilityCurve\n");

                Console.WriteLine("Association");
                Console.WriteLine($"\tPropertyId = {association.PropertyId.ToString()}");
                Console.WriteLine($"\tType = {association.Type.ToString()}");
                ...
                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\ReactiveCapabilityCurveAssignedToSynchronousMachine_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting ReactiveCapabilityCurve assigned to SynchronousMachine successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting ReactiveCapabilityCurve assigned to SynchronousMachine method  failed for sourceGlobalId = {0} and association (propertyId = {1}, type = {2}). Reason: {3}", sourceGlobalId, association.PropertyId, association.Type, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<long> GetSynchronousMachinesAssignedToReactiveCapabilityCurve()
        {

            long sourceGlobalId = GetFirstElement(ModelCode.REACTIVECAPABILITYCURVE, false);

            string message = "Getting SynchronousMachines assigned to ReactiveCapabilityCurve started.";
            ...
            Association association = new Association();
            association.PropertyId = ModelCode.REACTIVECAPABILITYCURVE_SYNCMAHINES;
            association.Type = 0;
            ...
            try
            {
                ...
                List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds(ModelCode.SYNCHRONOUSMACHINE);

                Console.WriteLine($"Getting values for all SynchronousMachinesProperties. ( number of properties = {properties.Count} )");
                CommonTrace.WriteTrace(CommonTrace.TraceError, $"Getting values for all SynchronousMachinesProperties. (number of properties = {properties.Count}");


                Console.WriteLine("\nReactiveCapabilityCurve -> 1..* SynchronousMachine\n");
                ...
                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\SynchronousMachinesAssignedToReactiveCapabilityCurve_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                message = "Getting SynchronousMachines assigned to ReactiveCapabilityCurve successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting SynchronousMachines assigned to ReactiveCapabilityCurve method  failed for sourceGlobalId = {0} and association (propertyId = {1}, type = {2}). Reason: {3}", sourceGlobalId, association.PropertyId, association.Type, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        #region GidHelpers
        private long GetFirstElement(ModelCode modelCode, bool max)
        {
            string message = "";
