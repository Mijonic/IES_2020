...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            ...
                            tgda.GetValuesForTapChangerControlWithMaxGid();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetValues failed. {0}", ex.Message);
                    ...
                    else if (str == "6")
                    {
                        try
                        {
                            ...
                            tgda.GetValuesForTerminalWithMinGid();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetValuesForTerminalWithMinGid failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    ...
                    else if (str == "7")
                    {
                        try
                        {
                            ...
                            tgda.GetExtentValuesForCurveData();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetExtentValuesForCurveData failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    ...
                    else if (str == "8")
                    {
                        try
                        {
                            ...
                            tgda.GetExtentValuesForSynchronousMachine();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetExtentValuesForSynchronousMachine failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    ...
                    else if (str == "9")
                    {
                        try
                        {
                            ...
                            tgda.GetReactiveCapabilityCurveForSyncMachine();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetReactiveCapabilityCurveForSyncMachine failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    ...
                    else if (str == "10")
                    {
                        try
                        {
                            ...
                            tgda.GetSynchronousMachinesAssignedToReactiveCapabilityCurve();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetSynchronousMachinesAssignedToReactiveCapabilityCurve failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get values for TapchangerControl with max gid");
            Console.WriteLine("\t6) Get values for Terminal with min gid");
            Console.WriteLine("\t7) Get extent values for CurveData");
            Console.WriteLine("\t8) Get extent values for SynchronousMachine");
            Console.WriteLine("\t9) Get ReactiveCapabilityCurve assigned to SynchronousMachine with min gid");
            Console.WriteLine("\t10) Get all SynchronousMachines assigned to ReactiveCapabilityCurve with min gid");
