...
namespace FTN.Services.NetworkModelService.DataModel.Core
{
    public class Curve : IdentifiedObject
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            {
                Curve x = (Curve)obj;
                return (x.curveStyle == this.curveStyle && x.xMultiplier == this.xMultiplier &&
                        x.xUnit == this.xUnit && x.y1Multiplier == this.y1Multiplier && x.Y1Unit == this.y1Unit &&
                        x.y2Multiplier == this.y2Multiplier && x.Y2Unit == this.y2Unit &&
                        x.y3Multiplier == this.y3Multiplier && x.Y3Unit == this.y3Unit &&
        ...
        public override bool HasProperty(ModelCode property)
        {
            switch (property)
            {
                ...
                case ModelCode.CURVE_CURVEDATA:
                    return true;
        ...
        public override void GetReferences(Dictionary<ModelCode, List<long>> references, TypeOfReference refType)
        {
            ...
            {
                references[ModelCode.CURVE_CURVEDATA] = curveDatas.GetRange(0, curveDatas.Count);
            }
